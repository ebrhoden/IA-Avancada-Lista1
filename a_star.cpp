...
Solution solve_a_star(vector<int> user_input, HeuristicCalculator h){
    ...
    vector<Puzzle*>* clean_up_helper = new vector<Puzzle*>;
    ...
    clean_up_helper->push_back(initial_puzzle_pointer);
    ...
    while(!open->empty()){
        ...
        if(closed->find(current_puzzle->state.internal_representation) == closed->end()){
            ...
            if(current_puzzle->state.is_goal()){
                ...
                for (auto p : *clean_up_helper){
                    delete p;
                }
                clean_up_helper->clear();
            ...
            for(State neighbor_state: neighbor_states){
                puzzle_counter++;
                Puzzle* neighbor_puzzle = new Puzzle(neighbor_state, current_puzzle, current_puzzle->depth + 1, h, puzzle_counter);
                open->push(neighbor_puzzle);
                clean_up_helper->push_back(neighbor_puzzle);
            }
        }
    }
